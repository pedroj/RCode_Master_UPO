...
```{r require, warning=FALSE, message=FALSE}
require(MASS)
require(datasets)
require(ade4)
...
```{r Renyi, eval=T, fig.width=10, fig.height=8}
data(BCI)
i <- sample(nrow(BCI), 9)
mod <- renyi(BCI[i,])
plot(mod)
...
```{r Rarefaction, eval=T, fig.width=10, fig.height=8}
...
```{r Species accumulation, eval=T, fig.width=10, fig.height=8}
sac <- specaccum(BCI)
...
```{r BiodiversityR, eval=T, warning=FALSE, message=FALSE, fig.width=10, fig.height=8}
...
```{r Asymptotic estimators, eval=T, fig.width=10, fig.height=8}
specpool(BCI)
```
...
Just do: 
```{r EstimateR #1,eval=T, fig.width=10, fig.height=8}
...
```{r EstimateR #2, eval=T, fig.width=10, fig.height=8}
